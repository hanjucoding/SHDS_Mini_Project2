...
$(document).ready(function() {
    ...
    // hidden input에서 roomInfo 가져오기
    roomInfo = $("#roomInfo").val();
    console.log("Room Info: " + roomInfo);

    // 이벤트 핸들러 등록
    $("#mycanvas").click(put_stone); // 캔버스 클릭 핸들러 등록
    $("#start_button").click(start_game); // 시작 버튼 클릭 핸들러 등록
    $("#retire_button").click(retire_game); // 기권 버튼 클릭 핸들러 등록
    $("#back_button").click(delete_stone); // 무르기 버튼 클릭 핸들러 등록
    $("#exit_button").click(exit_game); // 종료 버튼 클릭 핸들러 등록

    // 선수 변경 라디오 버튼 핸들러 등록
    //$("input[type=radio][name=who_first]").change(change_first);

    // 엘리먼트 초기 상태 지정
    $("#back_button").prop("disabled", true); // 무르기 버튼 비활성화
    $("#retire_button").prop("disabled", true); // 기권 버튼 비활성화
...
webSocket.onmessage = function(message) {
	console.log(message.data + "받아");
	if (message.data.includes("StonePlace : ")) {
		var getRoomInfo = "";
		var str = message.data.replace("StonePlace : ", ",");
		var data = str.split(",");
		getRoomInfo = data[0];
		var x = data[1];
		var y = data[2];
		is_myroom(getRoomInfo);
		console.log(getRoomInfo + " " + x + " " + y);
		board[y][x] = op_color;
		fillStone2(x, y, op_color);
		board_stack.push([x, y, op_color]);
		var result = check_pointer(x, y, op_color, board);
	...
	} else if (message.data.includes("Set Stone : ")) {
		my_stone = message.data.replace("Set Stone : ", "");
		if (my_stone == "white") {
			op_color = STONE_BLACK;
		}
		console.log(my_stone);
		console.log("상대 돌 색 " + op_color);
	} else if (message.data.includes("Set Current : ")) {
		current_stone = message.data.replace("Set Current : ", "");
...
function sendPosition(x, y) {
	var position = roomInfo + "StonePlace : " + x + "," + y;
...
// 현재 차례 질의
function is_myturn() {
	console.log("현재 차례 돌 : " + current_stone);
	console.log("내 돌 : " + my_stone);
	if (current_stone == my_stone) {
		console.log("내 차례");
		return true;
	} else {
		console.log("상대 차례");
...
function change_turn() {
	if (current_stone == "black") {
		webSocket.send(roomInfo + "Set Current : white");
		console.log("변경 된 차례 : " + current_stone);
	} else {
		webSocket.send(roomInfo + "Set Current : black");
		console.log("변경 된 차례 : " + current_stone);
	}
}
...
function win_game(_stone) {  // 게임 승리 관련 처리 루틴
    ...
    print_winmsg(_stone);
    end_game();
    webSocket.send("Game End At : " + roomInfo);
    if (my_stone == winner) {
        location.href='GameResult.do?result=win';
    } else {
        location.href='GameResult.do?result=lose';
    }
}
...
function start_game() {
	...
	console.log("Room Info: " + roomInfo);
    webSocket.send("Game Start At : " + roomInfo);
}
// 게임 기권 처리 루틴
...
function put_stone(e) {
	...
	if (!is_debugmode() && board[y][x] == STONE_NONE) {
		console.log(my_stone + " " + current_stone);
		var tmp_color = -1;
		op_color = -1;
